'''
main_sample_aiohttp.pyとの違いは，HTTPリクエストのリトライ時にセッションの再生成まで行うようにしていること．
こっちの方が，どのHTTPリクエストの通信が失敗してリトライされたかの標準エラー出力がしやすい．
'''
...
async def download_jsons_weather_forecast_async(
    ...
    async def run(
        ...
        try:
            ...
            raise type(e)("Failed in downloading from url '{url}'. Detail: {err}".format(
                url = url,
    ...
    for i in range(cnt_retry_req):
        try:
            ...
            err = type(e)("[TrialCnt={cnt}]{err}".format(
                cnt = i + 1,
                err = traceback.format_exc()
            ))
            ...
            if i == cnt_retry_req - 1:
                ...
                raise type(e)("Failed in downloading weather forecast after {cnt_retry} retrying.".format(
